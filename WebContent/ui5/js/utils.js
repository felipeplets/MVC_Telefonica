jQuery(function() {
	...
	if ( qterms ) {
		// create a styles map and regexp from the query terms
		styles = {};
		for(i=0; i<qterms.length; i++) {
	...
	function markText(node, expr, callback) {
		...
		function replace(node) {
			...
			while (m = expr.exec(text)) {

				// add a text node with the string before the match
				if ( start < m.index ) {
					p.insertBefore(document.createTextNode(text.slice(start, m.index)), node);
				}

				// add a span for the match
				span = document.createElement("span");
				span.appendChild(document.createTextNode(m[0]));
				span = callback.call(span, m[0]) || span;
